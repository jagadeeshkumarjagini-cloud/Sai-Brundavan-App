...
import { StyleSheet, Text, View, TouchableOpacity, ScrollView, Alert, SafeAreaView, Image, Modal, TextInput } from 'react-native';
...
export default function App() {
    ...
    <html><body style="font-family:Helvetica;padding:20px;color:#333;">
      ...
      <table style="width:100%;border-collapse:collapse;margin-top:20px;">
        ...
        <tbody style="font-size:10px;">
          ...
          <tr style="background:#f4f8ff;"><td>A1</td><td>-</td><td>Amt received through water tankers</td><td></td><td>₹${wR}</td><td>Pending: ₹${totWP}</td></tr>
          <tr style="background:#f4f8ff;"><td>B</td><td>-</td><td>Maintenance total (Current Month)</td><td></td><td>₹${mR}</td><td>Pending: ₹${totMP}</td></tr>
      ...
      <div style="margin-top:20px;border:1px solid #f87171;padding:10px;font-size:10px;">
        <p style="color:#d9534f;font-weight:bold;">RECEIVABLES SUMMARY:</p>
        <p>Water Pending: ${wP.map(p=>`F${p.flat}:₹${p.amt}`).join(', ') || 'Nil'}</p>
        <p>Maint. Pending: ${mP.map(p=>`F${p.flat}:₹${p.amt}`).join(', ') || 'Nil'}</p>
      </div>
  ...
  const scan = async () => {
    ...
    try {
      ...
      setVend(txt.split('\n')[0]?.substring(0,25) || 'Bill'); setAmt(bestAmt);
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView>
        ...
        <View style={styles.card}>
          <Text style={styles.sl}>Opening Bal (A)</Text><TextInput style={styles.in} keyboardType="numeric" value={oB} onChangeText={setOpening}/>
          <Text style={styles.sl}>Water Tankers (A1)</Text><TextInput style={styles.in} keyboardType="numeric" value={wR} onChangeText={setWR}/>
          <TouchableOpacity style={styles.add} onPress={()=>{setPT('Water');setPMV(true)}}><UserPlus size={12} color="#FFD700"/><Text style={styles.at}>Add Pending Water</Text></TouchableOpacity>
          <Text style={styles.sl}>Maintenance (B)</Text><TextInput style={styles.in} keyboardType="numeric" value={mR} onChangeText={setMR}/>
          <TouchableOpacity style={styles.add} onPress={()=>{setPT('Maint');setPMV(true)}}><UserPlus size={12} color="#FFD700"/><Text style={styles.at}>Add Pending Maint</Text></TouchableOpacity>
        </View>
      ...
      <View style={styles.footer}>
        <View style={styles.ac}><TouchableOpacity style={[styles.fb,{backgroundColor:'#1d2d50'}]} onPress={()=>setMV(true)}><Plus color="#FFD700"/></TouchableOpacity><Text style={styles.fl}>Add bills manually if any</Text></View>
      ...
      <Modal visible={pMV} transparent={true}><View style={styles.ov}><View style={styles.mc}>
        <Text style={{color:'#FFD700',fontWeight:'bold'}}>${pT} Pending</Text>
        ...
        <TouchableOpacity style={styles.sb} onPress={()=>{const n={id:Date.now(),flat:pF,amt:pA,mon:pM};pT==='Water'?setWaterP([...wP,n]):setMaintP([...mP,n]);setPMV(false)}}><Text style={{color:'#0A192F',fontWeight:'bold'}}>Save</Text></TouchableOpacity>
        <TouchableOpacity onPress={()=>setPMV(false)}><Text style={{color:'#8892b0',marginTop:10,textAlign:'center'}}>Go Back</Text></TouchableOpacity>
      </View></View></Modal>
      <Modal visible={mV} transparent={true} animationType="slide"><View style={styles.ov}><View style={styles.mc}>
        <View style={styles.ai}><Sparkles color="#FFD700" size={14}/><Text style={{color:'#FFD700',fontSize:10,marginLeft:5,flex:1}}>Check captured details are correct.</Text></View>
        <TextInput placeholder="Particulars" placeholderTextColor="#8892b0" style={styles.mi} value={vend} onChangeText={setVend}/><TextInput placeholder="Amount" placeholderTextColor="#8892b0" keyboardType="numeric" style={styles.mi} value={amt} onChangeText={setAmt}/><TextInput placeholder="Remarks" placeholderTextColor="#8892b0" style={styles.mi} value={rem} onChangeText={setRem}/>
        <TouchableOpacity style={styles.sb} onPress={()=>{setEx([{id:Date.now(),vendor:vend,amount:parseInt(amt)||0,remarks:rem,date:new Date().toLocaleDateString()},...expenses]);setMV(false)}}><Text style={{color:'#0A192F',fontWeight:'bold'}}>Verify & Save</Text></TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  logo: { width: 34, height: 34, borderRadius: 17, marginRight: 10, borderWidth: 1, borderColor: '#FFD700' },
